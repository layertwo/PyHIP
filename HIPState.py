...
class _StateMachine:
    ...
    def input(self, packet, h=None, rest=None):
        ...
        if (not h.type in [HIP_BOS, HIP_I1, HIP_PAYLOAD, HIP_I2]):
            ...
            try:
                ...
                ver = 1
            if not ver:
                # , repr(HI.HI.HITable[h.sourceHIT])
                print('Verify Failed', ver, hexlify(packet))
            else:
                if hasattr(self, 'trace'):
        ...
        if h.type in [HIP_BOS, HIP_I2]:
            ...
            try:
                ...
                ver = 1
            if not ver:
                # , repr(HI.HITable[h.sourceHIT])
                print('BOS Verify Failed', ver, hexlify(packet))
            else:
                if hasattr(self, 'trace'):
                    print(self.FQDN, 'Verified:', HIP_Packets[h.type])
        print('Valid?', m.valid)
        ...
        try:
            ...
            traceback.print_exc()
        if hasattr(self, 'trace'):
            print(self.FQDN, 'Transition:', next)
        self.setstate(next)
        try:
    ...
    def send(self, message):
        if hasattr(self, 'trace'):
            print(self.FQDN, repr(self.state), 'Sending:', message)
        message.valid = 1
