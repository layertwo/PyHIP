...
def unpackHeader(payload, object=None):
    ...
    if object is None:
        result['fqdn'] = payload[HIP_HEADER_LEN +
                                 2:HIP_HEADER_LEN + 2 + length]
    else:
...
def packetDump(packet):
    ...
    (nexthdr, paylen, type, magic, hit, rcount, fqdn) = [getattr(
        dict, x) for x in ('nh', 'length', 'type', 'magic', 'hit', 'rcount', 'fqdn')]
...
class I2Message(Message):
    ...
    def input(self, machine, header, RRset):
        ...
        try:
            ...
            # this isn't right
            raise HIPUnpackError(
                'Remote requested only unsupported transforms')
