...
class InputEngine(HIPutils.IPhandler):
        ...
        try:
            ...
            if handler.piggy:
                handler.piggy.send(HIPMessage.PAYLOAD,
                                   piggy=handler.piggy.remoteESP.pack(protocol, payload),
    ...
    def __init__(self, localIP=None, OutQueue=None):
        ...
        self.Socket = socket.socket(socket.AF_INET, socket.SOCK_RAW, socket.IPPROTO_ESP)
...
class SPI:
    ...
    def __init__(self, SPI,
        ...
        #if self.authkey: print 'Authkey:', hexlify(self.authkey), len(self.authkey), self.authoct
...
#        cipher=EVP.Cipher(self.alg, self.key, self.iv, enc)
        ...
        #print 'pack: Encoded:', len(plaintext), len(plaintext) % self.blocksize
...
##            pass
        ...
        #    macdata=''
        #print 'unpack: ciphertext of:', len(ciphertext), len(ciphertext)%self.blocksize
