...
class HostStateMachine(HIPState.StateMachine):
    ...
    def __init__(self, host, queue, interfaces, hit='', fqdn=''):
        # print 'New StateMachine:', fqdn, repr(hit), repr(self),
        # self.canpiggyback
...
# if x[0] in self.IPProtocols]
    ...
    def CMD_connect(self, sk, args):
        ...
        if hit in HI.HI.HITable:
            try:
                ...
                pass
            return 'Already connected to %s, with HIT %s.' % (
                host, hexlify(hit))
        if hi2 is not None:
    ...
    def makeRemoteSPI(self, remoteSPI, ESPalg,
                      remoteESPkey, remoteESPauthkey, ip):
        class Machine:
            pass
        Machine = Machine()
    ...
    def CMD_makeSPI(self, sk, args):
        ...
        try:
            ...
            return ''.join(traceback.format_exception(sys.exc_info()[0], sys.exc_info()[
                           1], sys.exc_info()[2]) + ['Wrong number of arguments'])
        try:
            ip = [x
                  for x in socket.getaddrinfo(host, None)
                  if x[0] in self.IPProtocols][0]
            ...
            return ''.join(traceback.format_exception(sys.exc_info()[0], sys.exc_info()[
                           1], sys.exc_info()[2]) + ['IP address invalid'])
        try:
            self.makeLocalSPI(
                SPI,
                ESPalg,
            ...
            return ''.join(traceback.format_exception(sys.exc_info()[0], sys.exc_info()[
                           1], sys.exc_info()[2]) + ['error making SPIs (wrong key length?)'])
    ...
    def handleScheduledEvents():
        try:
            ...
            while x < time.time():
                # print 'Event fired:', repr(self.FutureEvents[x]), 'set at',
                # x, 'now', time.time()
                self.FutureEvents[x]()
...
def main():
    ...
    print(
        ...
        hexlify(
            hi1.HIT64(
                unhexlify('9e800000000000000000000000000000'))))
